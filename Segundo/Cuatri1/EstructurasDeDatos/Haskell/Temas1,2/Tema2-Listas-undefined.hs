...
{-
   ...
   y otras son parciales.
      ...
      head :: [a] -> a Indica el primer elemento de la lista

      last :: [a] -> a Indica el ultimo elemento de la lista

      tail :: [a] -> [a] Devuelve toda la lista menos el primer elemento

      init :: [a] -> [a] Devuelve toda la lista menos el ultimo elemento
...
ordenada [] = True
ordenada [_] = False
ordenada (x:y:xs) = ordenada (y:ys) && x <= y
...
sumar' xs = sumarAc xs
   where
      sumarAc [] ac = ac
      sumarAc (x:xs) ac = sumarAc xs (x + ac)