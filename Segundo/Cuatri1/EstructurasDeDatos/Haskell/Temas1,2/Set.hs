...
insert x (Node y s) 
     | x == y = Node y s      --- Node x s ---- Esta se utilizaria en el caso de que se quisiera actualizar la informacion ya que se comparan los campos realmente relevantes
     | x < y = Node x (Node y s)
...
cardinal Empty = 0
cardinal (Node x s) = 1 + (cardinal s)
