...
module Feb2019.DataStructures.Graph.DictionaryWeightedGraph
  ( WeightedGraph
  , WeightedEdge(WE)
  , empty
  , isEmpty
...
addVertex (WG g) v = WG (D.insert v D.empty g)

addEdge :: (Ord a, Show a) => WeightedGraph a w -> a -> w -> a -> WeightedGraph a w
addEdge (WG g) x w y 
  | vertsInGraph (vertices (WG g)) x y = mkWeightedGraphEdges (vertices (WG g)) (edges ++ [(WE x w y)])
  | otherwise = error "Alguno de los vertices no esta en el grafo" 

vertsInGraph :: Eq a => [a] -> a -> a -> Bool
vertsInGraph verts x y = vertInGraph verts x && vertInGraph verts y
  where
    vertInGraph [] v = False 
    vertInGraph (x:xs) v  
      | v == x = True 
      | otherwise = vertInGraph xs v

edges :: (Eq a, Eq w) => WeightedGraph a w -> [WeightedEdge a w]
edges (WG g) = D.values g
