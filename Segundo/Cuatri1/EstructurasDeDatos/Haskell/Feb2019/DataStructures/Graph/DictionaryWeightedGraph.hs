...
--1. Sacar para cada vertice su diccionario valueOf 
--2. Crear aristas para cada elemento del diccionario
edges :: (Eq a, Eq w) => WeightedGraph a w -> [WeightedEdge a w]
edges g = aux (vertices g) 
  where
    aux [] = []
    aux (v:vs) = (vertEdges v g) ++ aux vs 
      where
        vertEdges v (WG g) = mkEdges (D.keys (Just (D.valueOf v g))) (Just (D.valueOf v g) )
          where
            mkEdges [] _ = []
            mkEdges (k:ks) dic = [WE v (valueOf k dic) k] ++ mkEdges ks dic
