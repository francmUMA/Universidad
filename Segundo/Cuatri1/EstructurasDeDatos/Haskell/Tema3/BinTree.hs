...
isElemB  x (NodeB y l r)
  | x == y = True
  | otherwise = isElemB x l || isElemB x r
...
atLevelB x EmptyB = []
atLevelB x (NodeB y l r)
  | x == 0 = [y]
  | otherwise = atLevelB (x-1) l ++ atLevelB (x-1) r
...
pathsToB e (NodeB x l r)
  |e == x = [[x]]
  | otherwise = map (x:) (pathsToB e l ++ pathsToB e r)
...
pathsToB2 x (NodeB y l r)
  | x == y = [[x]] ++ map (y:) (pathsToB2 x l ++ pathsToB2 x r)
