 ...
 where
  ...
  aux pendingPred
    ...
    where
      ...
      sucSrcs = concat [ successors g s | s <- srcs ]
      pendingPred' = foldr decrementValue dicWithoutSrcs sucSrcs -- restar los sucesores de las fuentes

list2Dic :: [(a,b)] -> Dictionary a b 
...
topSortings g  = aux initPenPred
  where
    ...
    aux pendingPred
      ...
      | otherwise  = undefined
      where
        srcs = undefined :: [a] -- list of vertices with inDegree 0
