...
void init_MMA8451(I2C_HandleTypeDef hi2c1){
    ...
    write_register(MMA8451_REG_PL_CFG, 0x40, hi2c1);

    // Modo activo con ruido reducido
    write_register(MMA8451_REG_CTRL_REG1, 0x01 | 0x04, hi2c1);

}

// Actualiza la orientación del acelerómetro
void get_orientation(mma8451_t *data, I2C_HandleTypeDef hi2c1){
    uint8_t *raw_data = read_register(MMA8451_REG_PL_STATUS, hi2c1);
...
mma8451_range_t get_range(I2C_HandleTypeDef hi2c1){
    return (mma8451_range_t) (read_register(MMA8451_REG_XYZ_DATA_CFG, hi2c1)[1] & 0x03);
}

// Actualiza los valores de los ejes del acelerómetro
void read(mma8451_t *data, I2C_HandleTypeDef hi2c1){
    // Actualizamos el valor de x
    uint8_t *x_msb = read_register(MMA8451_REG_OUT_X_MSB, hi2c1);
    uint8_t *x_lsb = read_register(MMA8451_REG_OUT_X_LSB, hi2c1);
    data -> x = x_msb[1];
    data -> x <<= 8;
    data -> x |= x_lsb[1];
    data -> x >>= 2;
    ...
    uint8_t *y_msb = read_register(MMA8451_REG_OUT_Y_MSB, hi2c1);
    uint8_t *y_lsb = read_register(MMA8451_REG_OUT_Y_LSB, hi2c1);
    data -> y = y_msb[1];
    data -> y <<= 8;
    data -> y |= y_lsb[1];
    data -> y >>= 2;
    ...
    uint8_t *z_msb = read_register(MMA8451_REG_OUT_Z_MSB, hi2c1);
    uint8_t *z_lsb = read_register(MMA8451_REG_OUT_Z_LSB, hi2c1);
    data -> z = z_msb[1];
    data -> z <<= 8;
    data -> z |= z_lsb[1];
    data -> z >>= 2;
...
void set_range(mma8451_range_t range, I2C_HandleTypeDef hi2c1){
    uint8_t reg = read_register(MMA8451_REG_CTRL_REG1, hi2c1)[1];
    write_register(MMA8451_REG_CTRL_REG1, 0x00, hi2c1);             // Se desactiva el acelerómetro
    write_register(MMA8451_REG_XYZ_DATA_CFG, range & 0x03, hi2c1);  // Se establece el nuevo range
    write_register(MMA8451_REG_CTRL_REG1, reg | 0x01, hi2c1);       // Se vuelve a activar el acelerómetro
}
